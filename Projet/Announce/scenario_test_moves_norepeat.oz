...
in
   scenario(bombLatency:3
	    ...
	    snakes: [
		     ...
		     snake(team:red name:john
			   positions: [pos(x:9 y:5 to:south) pos(x:9 y:4 to:south) pos(x:10 y:4 to:west) pos(x:11 y:4 to:west) pos(x:12 y:4 to:west)]
			   effects: nil
			   strategy: [forward forward turn(left) forward forward turn(left) forward forward turn(left) forward forward
                                      turn(left) forward forward turn(left) forward forward]
			   bombing: NoBomb
			  )
		    ]
	    bonuses: nil
